...
(function() {
  $(function() {
    ...
    window.hideAll = function() {
      visibleSections.forEach(function($nodes) {
        $nodes.removeClass("visible");
        $nodes.addClass("hidden");
        return hiddenSections.push($nodes);
      });
      return window.visibleSections = [];
    };
    window.setFragment = function(pageId) {
      return location.hash = pageId;
    };
    window.routeFromFragment = function() {
      var fragment;
      fragment = location.hash.replace("#", "");
      if (fragment.length > 0) {
        return $(".section-btn." + fragment).trigger("click");
      }
    };
    $.each($sectionBtns, function(idx, el) {
      ...
      return $el.on("click", function(e) {
        ...
        return setFragment($target.attr("id"));
      });
    });
